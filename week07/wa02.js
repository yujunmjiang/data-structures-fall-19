// Yujun Jiang
// Data Structure Weekly Assignment 07 (Pt. 02)
...
file.forEach(function(ele) {
    ...
    $('tr').each(function(i, elem) {
        if ($(elem).attr('style') === "margin-bottom:10px") {
            ...
            var thisMeeting = {};
            // thisMeeting{group: , building: , address: , city: , state: , zip: , accessbility: , allMeetings: []}
            ...
            if ($(elem).html().split('</h4>')[0].trim().split("<h4 style=\"margin:0;padding:0;\">")[1].length > 0) {
                building = $(elem).html().split('</h4>')[0].trim().split("<h4 style=\"margin:0;padding:0;\">")[1];
            }
            else {
                building = "N/A";
            }
            thisMeeting.building = building;
...
fs.writeFileSync('/home/ec2-user/environment/week07/data/data-manhattan.json', JSON.stringify(meetings));